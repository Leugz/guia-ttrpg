...
import { ResourceMathInput } from './features/character-sheet/components/ResourceMathInput';
...
export default function App() {
  ...
  const handleLoadCharacter = async (fileName: string) => {
    const fullPath = `/home/leugz_/Projects/personal/guia/campaigns/act_1/templates/${fileName}`;
    try {
      ...
      <div className='pointer-events-none absolute left-0 top-0 z-10 flex w-full items-start justify-between p-4'>
        ...
        <div className='pointer-events-auto flex flex-col items-end gap-2'>
          ...
          >
            ...
            <button className='ml-2 text-zinc-500 hover:text-white'>
              <Settings size={14} />
            </button>
          </div>
          <div className='flex gap-2'>
            <div
              className='flex h-8 w-8 cursor-pointer items-center justify-center rounded-sm border border-zinc-700 bg-zinc-800 text-xs font-bold text-zinc-400 shadow-lg hover:border-zinc-500'
              title='GM Kaiser (Host)'
            >
              GM
            </div>
            <div
              className='flex h-8 w-8 cursor-pointer items-center justify-center rounded-sm border border-blue-900/50 bg-zinc-900 text-xs font-bold text-blue-500 shadow-lg hover:border-blue-500'
              title='Player 2 (Conectado)'
            >
              P2
            </div>
          </div>
