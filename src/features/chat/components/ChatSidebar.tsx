...
export function ChatSidebar() {
  ...
  const filteredMessages = messages.filter((msg) => {
    if (filter === 'messages') {
      return msg.type === 'text' && !msg.rollLabel;
    }
    if (filter === 'dice') {
    ...
    <aside className='relative z-10 flex w-[320px] flex-col border-r border-neutral-700 bg-[#121212]'>
      ...
      {!isAtBottom && (
        <button
          ...
          className='absolute bottom-6 left-1/2 -translate-x-1/2 rounded-full bg-blue-600 px-4 py-2 text-xs font-bold text-white shadow-lg transition-transform hover:scale-105'
        >
          ↓ Novas Mensagens
        </button>
      )}
